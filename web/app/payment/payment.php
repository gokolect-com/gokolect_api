...
$prefix = 'GK'; // Change this to the name of your business or app
$overrideRef = false;

//Uncomment here to enforce the useage of your own ref else a ref will be generated for you automatically
if (isset($postData['payRef'])) {
    $prefix.= $postData['payRef'];
    $overrideRef = true;
}
 ...
 * @param object $data the transaction details.
 * 
 * @return string
 */
...
class MyEventHandler extends DataOps implements EventHandlerInterface
{
    
    /**
     * This is called when the Rave class is initialized
     * 
     * @param object $initializationData transaction data.
     * 
     * @return mix
     */
    function onInit($initializationData) 
    {
        ...
        if (dataOps::getConnection()) {
            $result = dataOps::save($initializationData);            
            $response = $result;
        }
        return $response;
    }
        ...
        if ($transactionData->status === 'successful') {
            if ($transactionData->currency == $_SESSION['currency'] && $transactionData->amount == $_SESSION['amount']) {
                ...
                if ($_SESSION['publicKey']) {
                    $u = (array) explode('?', $_SESSION['successfulurl']);
                    $url = $u[1]."&event=successful";
                    header('Location: ' . $url);
                ...
                if ($_SESSION['publicKey']) {
                    $u = (array) explode('?', $_SESSION['failureurl']);
                    $url = $u[1]."&event=suspicious";
                    header('Location: ' . $url);
    ...
    /**
     * This is called when a transaction is re-queried from the payment gateway.
     * 
     ...
     * 
     * @param object $requeryResponse re-query response.
     * 
     * @return mixed
     */
    ...
    function onCancel($transactionReference) 
    {
        ...
        // die(var_dump(getURL($_SESSION['failureurl'], array('event' => 'canceled')), $_SESSION));
        if ($_SESSION['publicKey']) {
            $u = (array) explode('?', $_SESSION['failureurl']);
            $url = $u[1]."&event=canceled";
            header('Location: ' . $url);
    ...
    /**
     * This is called when a transaction doesn't 
     * return with a success or a failure response. This can   
     * be a timed out transaction on the Rave server or an abandoned 
